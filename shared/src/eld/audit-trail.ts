...
export const EDIT_REASON_DESCRIPTIONS: Record<EditReasonCode, string> = {
...
// ─────────────────────────────────────────────────────────────────────────────
// AuditMetadata — the full audit object embedded on every event record
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export const GENESIS_SALT = 'cruzonic-eld-v1';
 ...
 */
export function canonicaliseEventFields(fields: HashableEventFields): string {
  const ordered: Record<string, unknown> = {};
  for (const key of (Object.keys(fields) as (keyof HashableEventFields)[]).sort()) {
 ...
 */
export function computeGenesisHash(
  eldDeviceId: string,
  logDate: string,
  hash: (input: string) => string,
): string {
  return hash(`${eldDeviceId}:${logDate}:${GENESIS_SALT}`);
}
...
export type ChainVerificationCode =
  | 'CONTENT_HASH_MISMATCH'     // event data was modified without updating the hash
  | 'CHAIN_HASH_MISMATCH'       // chainHash doesn't link correctly to previous
  | 'GENESIS_HASH_MISMATCH'     // first record's previousChainHash doesn't match expected genesis
  | 'MISSING_PREVIOUS_HASH'     // non-first record has null previousChainHash
  | 'VERSION_GAP'               // version numbers are not sequential
  | 'FUTURE_HASH_TIMESTAMP'     // hashedAt is after the current time (clock tampering)
  | 'CHAIN_OK';                 // no issues found
...
export interface ChainVerificationResult {
  ...
  summary: { TAMPER: number; WARN: number; INFO: number };
}
 ...
 */
export function verifyHashChain<T extends { eventSequenceId: number; id: string; audit: AuditMetadata }>(
  activeEvents: T[],
  eldDeviceId: string,
  logDate: string,
  hash: (input: string) => string,
  recomputeContent?: (event: T) => HashableEventFields,
): ChainVerificationResult {
  const findings: ChainVerificationFinding[] = [];
  const now = Date.now();
  ...
  let expectedPreviousChainHash: string = expectedGenesisHash;
  ...
  for (let i = 0; i < activeEvents.length; i++) {
    ...
    if (recomputeContent) {
      ...
      if (recomputed !== te.contentHash) {
        findings.push({
          ...
          message:
            `Content hash mismatch for event ${event.eventSequenceId}. ` +
    ...
    if (i === 0 && te.previousChainHash !== null && te.previousChainHash !== expectedGenesisHash) {
      findings.push({
        ...
        message:
          `First record's previousChainHash (${te.previousChainHash}) does not match ` +
    ...
    if (expectedChainHash !== te.chainHash) {
      findings.push({
        ...
        message:
          `Chain hash mismatch for event ${event.eventSequenceId}. ` +
    ...
    if (hashedAtMs > now + 60_000) { // allow 60-second clock skew
      findings.push({
        ...ref,
        severity: 'WARN',
 ...
 */
export function buildInitialAuditMetadata(
  eventId: string,
  actor: AuditActor,
  fields: HashableEventFields,
  prevHash: string,
  logDate: string,
  hash: (input: string) => string,
  network: NetworkContext | null = null,
 ...
 */
export function buildEditedAuditMetadata(
  newEventId: string,
  previousVersionId: string,
  previousAudit: AuditMetadata,
  actor: AuditActor,
  changedFields: FieldDiff[],
  reasonCode: EditReasonCode,
  reasonText: string | null,
  newFields: HashableEventFields,
  ...
  if (reasonCode === EditReasonCode.OTHER && (!reasonText || reasonText.length < 20)) {
    throw new Error(
      'editReasonText is required and must be ≥20 characters when editReasonCode is OTHER.',
    );
  }
  ...
  const requiresDriverReview =
    actor.type === AuditActorType.CARRIER ||
 ...
 * as INACTIVE_CHANGED.
 */
export function recordDriverReview(
  existingAudit: AuditMetadata,
  eventId: string,
  driver: AuditActor,
 ...
 */
function generateAuditEntryId(): string {
